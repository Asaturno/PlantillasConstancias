...
class GestorSuperusuarios(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        form_frame = ttk.LabelFrame(main_frame, text="Formulario de Superusuario", padding="10")
        form_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(form_frame, text="Nombre:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.nombre_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.nombre_var).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        ttk.Label(form_frame, text="Contraseña:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.contrasena_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.contrasena_var, show="*").grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        
        ttk.Label(form_frame, text="Confirmar Contraseña:").grid(row=2, column=0, sticky="e", padx=5, pady=5)
        self.confirmar_contrasena_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.confirmar_contrasena_var, show="*").grid(row=2, column=1, padx=5, pady=5, sticky="ew")
        ...
        list_frame = ttk.LabelFrame(main_frame, text="Superusuarios Registrados", padding="10")
    ...
    def _guardar_superusuario(self):
        ...
        if not nombre:
            messagebox.showwarning("Campo vacío", "Debes ingresar un nombre de usuario.", parent=self)
            return
            
        if not contrasena or not confirmar_contrasena:
            messagebox.showwarning("Campo vacío", "Debes ingresar y confirmar la contraseña.", parent=self)
            return
            
        if contrasena != confirmar_contrasena:
            messagebox.showwarning("Error", "Las contraseñas no coinciden.", parent=self)
            return
            
        if len(contrasena) < 6:
            messagebox.showwarning("Error", "La contraseña debe tener al menos 6 caracteres.", parent=self)
            return
            
        if not self.modo_edicion:
            self.cursor.execute("SELECT id FROM usuarios WHERE nombre = ? AND es_superusuario = 1", (nombre,))
            if self.cursor.fetchone():
                messagebox.showwarning("Error", "Ya existe un superusuario con ese nombre.", parent=self)
                return
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudo guardar el superusuario:\n{str(e)}", parent=self)

    def _cargar_superusuarios(self):
        for item in self.tree.get_children():
            self.tree.delete(item)
            
        self.cursor.execute("SELECT id, nombre FROM usuarios WHERE es_superusuario = 1 ORDER BY nombre")
    ...
    def _editar_superusuario(self):
        ...
        if not seleccion:
            messagebox.showwarning("Selección requerida", "Debes seleccionar un superusuario para editar.", parent=self)
            return
    ...
    def _eliminar_superusuario(self):
        seleccion = self.tree.selection()
        if not seleccion:
            messagebox.showwarning("Selección requerida", "Debes seleccionar un superusuario para eliminar.", parent=self)
            return
            
        superusuario_id = self.tree.item(seleccion[0])['values'][0]
        
        if superusuario_id == 1:
            messagebox.showwarning("Error", "No se puede eliminar el superusuario principal 'admin'.", parent=self)
            return
            
        confirm = messagebox.askyesno("Confirmar eliminación", "¿Estás seguro de que deseas eliminar este superusuario?", parent=self)
        
        if confirm:
            try:
                self.cursor.execute("DELETE FROM usuarios WHERE id = ? AND es_superusuario = 1", (superusuario_id,))
                self.conn.commit()
                messagebox.showinfo("Éxito", "Superusuario eliminado correctamente", parent=self)
                self._cargar_superusuarios()
            except sqlite3.Error as e:
                messagebox.showerror("Error", f"No se pudo eliminar el superusuario:\n{str(e)}", parent=self)
