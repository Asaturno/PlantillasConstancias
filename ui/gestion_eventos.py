...
class GestorEventos(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        form_frame = ttk.LabelFrame(
            main_frame, text="Formulario de Evento", padding="10")
        form_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Label(form_frame, text="Nombre del evento:").grid(
            row=0, column=0, sticky="e", padx=5, pady=5)
        self.nombre_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.nombre_var, font=('Arial', 10)).grid(
            row=0, column=1, padx=5, pady=5, sticky="ew")

        ttk.Label(form_frame, text="Fecha (AAAA-MM-DD):").grid(row=1,
                                                               column=0, sticky="e", padx=5, pady=5)
        self.fecha_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.fecha_var, font=('Arial', 10)).grid(
        ...
        list_frame = ttk.LabelFrame(
            main_frame, text="Eventos Registrados", padding="10")
    ...
    def _guardar_evento(self):
        ...
        if not nombre or not fecha:
            messagebox.showwarning(
                "Campos vacíos", "Debes ingresar nombre y fecha.")
            return
        ...
        if self.modo_edicion and self.registro_id is not None:
            ...
            self.cursor.execute(
                "INSERT INTO eventos (nombre, fecha) VALUES (?, ?)", (nombre, fecha))
            self.conn.commit()
    ...
    def _eliminar_evento(self):
        ...
        confirm = messagebox.askyesno(
            "Confirmar", "¿Estás seguro de eliminar este evento?")
        ...
        if confirm:
            try:
                ...
                messagebox.showerror(
                    "Error", "No se puede eliminar el evento porque tiene constancias asociadas")
