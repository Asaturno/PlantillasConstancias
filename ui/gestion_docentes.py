...
class GestorDocentes(tk.Toplevel):
    ...
    def _configurar_estilos(self):
        ...
        self.style.configure('TLabel', background='#f5f5f5', foreground='#333333', font=('Arial', 10))
    ...
    def _build_ui(self):
        ...
        form_frame = ttk.LabelFrame(main_frame, text="Formulario de Docente", padding="10")
        form_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # Campos del formulario
        ttk.Label(form_frame, text="Nombre completo:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        ...
        ttk.Label(form_frame, text="Grado académico:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.grado_var = tk.StringVar()
        grado_entry = ttk.Entry(form_frame, textvariable=self.grado_var, width=40)
        ...
        # Lista de docentes
        list_frame = ttk.LabelFrame(main_frame, text="Docentes Registrados", padding="10")
    ...
    def _guardar_docente(self):
        ...
        if not nombre or not grado:
            messagebox.showwarning("Campos vacíos", "Debes ingresar nombre y grado.", parent=self)
            return
        ...
        for row in self.cursor.fetchall():
            ...
            if existente_normalizado == nuevo_normalizado and (not self.modo_edicion or row[0] != self.registro_id):
                messagebox.showwarning("Duplicado", "Ya existe un docente con el mismo nombre y grado.", parent=self)
                return
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudo guardar el docente:\n{str(e)}", parent=self)
    ...
    def _editar_docente(self):
        ...
        if not seleccion:
            messagebox.showwarning("Selección requerida", "Debes seleccionar un docente para editar.", parent=self)
            return
    ...
    def _eliminar_docente(self):
        ...
        seleccion = self.tree.selection()
        if not seleccion:
            messagebox.showwarning("Selección requerida", "Debes seleccionar un docente para eliminar.", parent=self)
            return
        ...
        if confirm:
            try:
                ...
                self.conn.commit()
                messagebox.showinfo("Éxito", "Docente eliminado correctamente", parent=self)
                self._cargar_docentes()
