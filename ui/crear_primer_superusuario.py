...
class CrearPrimerSuperusuario(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(form_frame, textvariable=self.nombre_var).grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        
        # Contraseña
        ttk.Label(form_frame, text="Contraseña:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.contrasena_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.contrasena_var, show="*").grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        
        # Confirmar Contraseña
        ttk.Label(form_frame, text="Confirmar:").grid(row=2, column=0, sticky="e", padx=5, pady=5)
        self.confirmar_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=self.confirmar_var, show="*").grid(row=2, column=1, sticky="ew", padx=5, pady=5)
    ...
    def _crear_superusuario(self):
        ...
        if not contrasena or not confirmar:
            messagebox.showwarning("Error", "Debe ingresar y confirmar la contraseña")
            return
        ...
        try:
            ...
            if cursor.fetchone()[0] > 0:
                messagebox.showwarning("Error", "Ya existe un superusuario en el sistema")
                return
